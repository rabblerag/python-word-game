...
def main(player = None):
    ...
    try:
        with open(programpath + "assets\\" + "config.yaml", "r") as f: config = yaml.load(f, Loader=yaml.FullLoader)
    except FileNotFoundError:
        with open(programpath + "assets\\" + "config.yaml", "w") as f: 
    ...
    music= pyglet.media.load(programpath + "assets\\" +"jazzy-abstract-beat-11254.mp3", streaming=False) #StaticSource object
...
##### MUSIC VOLUME FUNCTIONS  ######
    ...
    def exit_music(self):
        ...
        with open(programpath + "assets\\" + "config.yaml", "w") as f: yaml.dump(config, f)
        self.window.destroy()
